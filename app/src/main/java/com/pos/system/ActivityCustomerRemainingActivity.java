...
import com.pos.system.databinding.ActivityCustomerRemainingBinding;

public class ActivityCustomerRemainingActivity extends BaseActivity {

    private ActivityCustomerRemainingBinding binding;
    private DBHelper dbHelper;
    private String currency = "ج.م";
    private List<HashMap<String, String>> customers = new ArrayList<>();
    private DebtAdapter adapter;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = ActivityCustomerRemainingBinding.inflate(getLayoutInflater());
    ...
    private void loadData() {
        customers.clear();
        customers.addAll(dbHelper.getCustomersWithDebt());
        double total = 0;
        for (HashMap<String, String> c : customers) {
            try { total += Double.parseDouble(c.getOrDefault("debt", "0")); } catch (Exception ignored) {}
        }
    ...
    private void showPayDialog(HashMap<String, String> customer) {
        long id = 0;
        double debt = 0;
        String name = customer.getOrDefault("name", "");
        try { id   = Long.parseLong(customer.getOrDefault("id",   "0")); } catch (Exception ignored) {}
        try { debt = Double.parseDouble(customer.getOrDefault("debt", "0")); } catch (Exception ignored) {}
        final long fId = id;
        ...
        new MaterialAlertDialogBuilder(this)
            .setTitle(getString(R.string.collect_from_customer_format, name,
            ...
            .setPositiveButton(R.string.confirm, (d, w) -> {
                try {
                    ...
                    dbHelper.addDirectPayment("customer", fId, name, "IN", paid, getString(R.string.debt_payment_note), date);
                    loadData();
    ...
    private class DebtAdapter extends RecyclerView.Adapter<DebtAdapter.VH> {
        ...
        public void onBindViewHolder(@NonNull VH h, int pos) {
            ...
            if (h.tvName  != null) h.tvName.setText(c.getOrDefault("name", ""));
            if (h.tvPhone != null) h.tvPhone.setText(c.getOrDefault("phone", ""));
            if (h.tvDebt  != null) h.tvDebt.setText(String.format(Locale.US, "%.2f %s", debt, currency));
            if (h.btnPay  != null) h.btnPay.setOnClickListener(v -> showPayDialog(c));
        }
